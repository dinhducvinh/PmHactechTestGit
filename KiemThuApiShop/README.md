...
- `Search`: save search, get list saved search.
- `FollowBlock`: follow/unfollow, list followed/following, block/unblock, list blocks.
- `DevTokenPush`: set device token, get/set push setting.
...
dotnet run -- --group Search
dotnet run -- --group FollowBlock
dotnet run -- --group DevTokenPush
dotnet run -- --case AUTH-LOGIN-01
dotnet run -- --case AUTH-LOGIN-01,USER-GET-INFO-01
...
- `tuDongChuanBiDuLieu`: tự signup tài khoản seed, tạo saved search/follow/block mồi tốt nhất có thể.
...
- Tạo dữ liệu tìm kiếm đã lưu vào `tk_timkiem_seed` bằng `/api/save_search`.
- Tạo 2-3 quan hệ follow của một `follower_chinh` vào `tk_theodoi_seed` bằng `/set_user_follow`.
- Tạo 2-3 quan hệ block của một `blocker_chinh` vào `tk_chan_seed` bằng `/set_user_block`, tránh nhóm đang được follow để không làm hỏng tiền đề follow.

Những case cần điều kiện chưa thể tạo chỉ bằng API public, ví dụ rule khóa tài khoản, sẽ được đánh dấu `BoQua` với lý do tiếng Việt thay vì báo sai server.
