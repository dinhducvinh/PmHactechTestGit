...
public static partial class BoKichBanApi
{
    ...
    private static void ThemLayThongTinNguoiDung(List<KichBanApi> ds)
    {
        ...
        Them(ds, "USER-GET-INFO-03", "User", "User A xem hồ sơ public của User B",
            ...
            async ctx =>
            {
                ...
                var token = await ctx.YeuCauTokenCuaTaiKhoanAsync(nguoiXem);
                return new YeuCauApi(
                    HttpMethod.Post,
                    "/users/get_user_info",
                    Obj(("user_id", SoIdBatBuoc(nguoiDuocXem.TaiKhoanIdServer, "tk_id_server user B"))),
                    token);
            },
            Ok,
        ...
        Them(ds, "USER-GET-INFO-04", "User", "Xem user đang được current user theo dõi",
            ...
            async ctx =>
            {
                ...
                var token = await ctx.YeuCauTokenCuaTaiKhoanAsync(capTheoDoi.TaiKhoanTheoDoi);
                return new YeuCauApi(HttpMethod.Post, "/users/get_user_info", Obj(("user_id", SoIdBatBuoc(capTheoDoi.TaiKhoanDuocTheoDoi.TaiKhoanIdServer, "followee_tk_id_server"))), token);
            },
            Ok,
        ...
        Them(ds, "USER-GET-INFO-05", "User", "Xem user đã bị current user chặn",
            ...
            async ctx =>
            {
                ...
                var token = await ctx.YeuCauTokenCuaTaiKhoanAsync(capChan.TaiKhoanChan);
                return new YeuCauApi(HttpMethod.Post, "/users/get_user_info", Obj(("user_id", SoIdBatBuoc(capChan.TaiKhoanBiChan.TaiKhoanIdServer, "blocked_tk_id_server"))), token);
            },
    ...
    private static Func<PhanHoiApi, YeuCauApi, NguCanhKiemThu, Task<KetQuaKiemTraThem>> KiemTraHoSoPublicNguoiKhac()
    {
        return (response, _, _) =>
        {
            ...
            {
                if (TienIchJson.CoTruong(response.Data, truongRiengTu))
                {
    ...
    private static KetQuaKiemTraThem KiemTraCoTruong(PhanHoiApi response, params string[] truongBatBuoc)
    {
        ...
        {
            return new KetQuaKiemTraThem(false, "Response thiếu data.");
        }

        foreach (var truong in truongBatBuoc)
        {
            if (!TienIchJson.CoTruong(response.Data, truong))
            {
